...
import com.yeemos.app.interfaces.ICameraHelper;
...
//            setUpPicSize(camera);
    ...
    public void setUpPicSize(Camera camera) {
        ...
        try {
            Camera.Size adapterSize = findBestResolution(camera, 9f / 16, TYPE_PICTURE);
    ...
    public void setUpPreviewSize(Camera camera) {
        ...
        try {
            Camera.Size adapterSize = findBestResolution(camera, 9f / 16, TYPE_PREVIEW);
     ...
     */
    public Camera.Size findBestResolution(Camera cameraInst, final float maxDistortion, int type) throws Exception {
        Camera.Parameters cameraParameters = cameraInst.getParameters();
        List<Camera.Size> supportedPicResolutions = type == TYPE_PREVIEW ? cameraParameters.getSupportedPreviewSizes() : cameraParameters.getSupportedPictureSizes(); // 至少会返回一个值
        ...
        Collections.sort(sortedSupportedPicResolutions, new Comparator<Camera.Size>() {
            ...
            public int compare(Camera.Size a, Camera.Size b) {
                float aRatio = a.width / a.height;
                float bRatio = b.width / b.height;

                if (Math.abs(aRatio - maxDistortion) < Math.abs(bRatio - 1)) {
                    return 1;
                } else if (Math.abs(aRatio - maxDistortion) == Math.abs(bRatio - 1)) {
                    return 0;
                } else {
                    return -1;
                }
        ...
        Log.e("aaaaaaaa", sortedSupportedPicResolutions.get(0).width + ", " + sortedSupportedPicResolutions.get(0).height);
        //返回最合适的
