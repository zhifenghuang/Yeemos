...
//    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mCurItem == 0 && mRefrshView != null) {
            if (mFirstY <= 0) {
                mFirstY = (int) ev.getY();
            }

            if (mMoveType == 0) {
                int minDis = ViewConfiguration.get(getContext()).getScaledDoubleTapSlop();
                if (ev.getY() - mFirstY > minDis) {
                    mMoveType = 1;
                } else if (ev.getY() - mFirstY < -minDis) {
                    mMoveType = 2;
                }
            } else if (mMoveType == 1) {
                mRefrshView.onTouchEvent(ev);
            }
        }
